...
aoc_harness::aoc_main!(2020 day 18, part1 [p1], part2 [p2]);
...
impl StackFrame {
    ...
    fn collapse_to(self, other : &mut Self) {
        assert_eq!(self.op,Op::None);
        other.apply(self.val);
    }
    fn apply(&mut self, val: i64) {
        match self.op {
            Op::None => {
                if self.val == 0 {
                    self.val = val;
    ...
    fn pending_multiply(&mut self) {
        assert_eq!(self.op,Op::None);
        self.op = Op::Mul;
    }
    fn pending_addition(&mut self) {
        assert_eq!(self.op,Op::None);
        self.op = Op::Add;
    }
}
...
fn solve(input: &str, mode: Mode) -> i64 {
    input
        .lines()
        .map(|l| {
