...
aoc_main!(2020 day 19, generator gen, part1 [p1], part2 [p2]);
...
fn matches(s: &str, rules: &[Rule]) -> bool {
...
fn p1((rules, input): &(Vec<Rule>, Vec<String>)) -> usize {
    input.iter().filter(|l| matches(l, rules)).count()
}

fn p2((rules, input): &(Vec<Rule>, Vec<String>)) -> usize {
