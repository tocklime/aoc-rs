...
fn p2_fast(input: &str) -> usize {
    let mut g = Grid2d::from_str(input, |x| x);
    let mut removed = 0;
    let mut to_remove: FxHashSet<Point<usize>> =
        g.indexes().filter(|p| is_removable(&g, *p)).collect();
...
/// In this solution we represent the grid as a grid of numbers.
