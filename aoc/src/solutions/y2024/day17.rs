...
impl Machine {
    ...
    fn run_with_a(&mut self, init_a: i64) {
        self.regs[0] = init_a;
        self.run();
    }
    fn match_start(&self) -> usize {
...
fn p2_backwards(m: &Machine) -> i64 {
    let mut stack: Vec<(i64, i64, usize)> = [(0, 0, 0)].into_iter().collect();
    while let Some((s, next_digit_to_try, step_ix)) = stack.pop() {
        if next_digit_to_try < 7 {
            stack.push((s, next_digit_to_try + 1, step_ix));
        }
        //considering adding `next_digit_to_try` as new octal digit on the end (lsb) of the value.
        //it's good if it outputs at least the last `step_ix` digits.
        let candidate = s << 3 | next_digit_to_try;
        let mut m = m.clone();
        m.run_with_a(candidate);
        let score = m.match_end();
        if score == m.program.len() {
            return candidate;
        }
        if score >= step_ix {
            stack.push((candidate, 0, step_ix + 1));
        }
    }
    0
}
