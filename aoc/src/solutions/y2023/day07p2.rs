...
aoc_harness::aoc_main!(2023 day 7, part2 [p2], example part2 EG => 5905);
...
fn detect_type(a: &str) -> HandType {
    let mut map : counter::Counter<char> = a.chars().collect();
    let j_count : usize = map.remove(&'J').unwrap_or_default();
    let mut scores : Vec<usize> = map.values().sorted().rev().copied().collect();
    if !scores.is_empty() {
        scores[0] += j_count;
    } else {
        scores.push(j_count);
