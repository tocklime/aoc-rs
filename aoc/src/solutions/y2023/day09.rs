...
aoc_harness::aoc_main!(2023 day 9, part1 [p1], part2 [p2], example both EG => (114,2));
...
fn p1(input: &str) -> i64 {
    input
        ...
        .map(|l| {
            ...
            let pred : i64 = grid.iter().rev().map(|x| x.last().unwrap()).sum();
            pred
        })
        .sum()
}
...
fn p2(input: &str) -> i64 {
    input
        ...
        .map(|l| {
            ...
            let pred : i64 = grid.iter().rev().fold(0, |acc, x| x.first().unwrap() - acc);
            println!("{l}");
            for g in grid {
                println!("{}",g.iter().map(|x| x.to_string()).join(" "));
            }
            println!("{pred}\n\n");
            pred
        })
