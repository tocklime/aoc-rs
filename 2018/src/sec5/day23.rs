...
impl Nanobot {
    fn parse(input: &str) -> Self {
        ...
        Self {
            pos: [c[1].parse().unwrap(), c[2].parse().unwrap(), c[3].parse().unwrap()],
            r: c[4].parse().unwrap(),
        }
    }
...
fn bron_kerbosch<'a, T>(neighbours: &HashMap<&'a T, HashSet<&'a T>>) -> HashSet<&'a T>
    ...
    fn recur<'a, 'b,T>(neighbours: &'a HashMap<&'b T, HashSet<&'b T>>, best: &'a mut Cell<HashSet<&'b T>>, p: HashSet<&'b T>, r: HashSet<&'b T>, x: HashSet<&'b T>)
        where T: Eq + Hash {
        if p.is_empty() && x.is_empty() {
            ...
            let most_neighbours_of_p_and_x = p.union(&x).max_by_key(|&&x| neighbours[x].len()).unwrap();
            for n in p.difference(&neighbours[most_neighbours_of_p_and_x]) {
                ...
                recur(
                    neighbours, best,
                    p.intersection(ns).map(|&x|x).collect(),
                    new_r,
                    x.intersection(ns).map(|&x|x).collect(),
                );
            }
        }
    }
    recur(neighbours, &mut best, neighbours.keys().map(|&x|x).collect(), HashSet::new(), HashSet::new());
    return best.into_inner();
}
...
fn p2(input: &str) -> N {
    ...
    for v in bots.iter().combinations(2) {
        ...
        if a.shares_space_with(b) {
            shares_space_with.entry(&a).or_insert(HashSet::new()).insert(b);
            shares_space_with.entry(&b).or_insert(HashSet::new()).insert(a);
        }
    }
    let maximum_clique = bron_kerbosch(&shares_space_with);
    maximum_clique.iter().map(|x| x.distance_to_origin() - x.r).max().unwrap()
}
...
impl Cube {
    ...
    fn octants(&self) -> Vec<Self> {
        ...
        (0..3).map(|_| (0..=1)).multi_cartesian_product().map(|v| {
            let new_lo:Vec<i64> = self.lo.iter().zip(v.into_iter()).map(|(l,d)| l + d * new_w).collect();
...
fn p2b(input: &str) -> N {
    ...
    work.push((bots.len(),init_cube.size(),-init_cube.distance_to_origin(),init_cube));
    while !work.is_empty() {
