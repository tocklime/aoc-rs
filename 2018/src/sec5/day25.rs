...
#[aoc(day25, part1,zzz_slow)]
fn p1(input: &str) -> usize {
    let neighbours = gen(input);
    let mut root_to_members: HashMap<usize, HashSet<usize>> = (0..neighbours.len()).map(|x| (x, [x].iter().cloned().collect::<HashSet<usize>>())).collect();
    loop {
        let found_to_join: Option<(usize, usize)> = root_to_members.iter().combinations(2)
            .find_map(|v| {
                if v[0].1.iter().any(|s| neighbours[s].intersection(v[1].1).nth(0).is_some())
                {
                    Some((*v[0].0, *v[1].0))
                } else { None }
            }
...
fn gen(input: &str) -> Vec<HashSet<usize>> {
    let coords: Vec<Vec<i64>> = input.lines().map(|l| l.split(',').map(|n| n.parse().unwrap()).collect()).collect();
...
fn p1b(input: &str) -> usize {
    ...
    for c in 0..neighbours.len() {
        ...
        while !fringe.is_empty() {
            ...
            fringe = fringe.iter().flat_map(|x| neighbours[x].iter().filter(|x| !visited.contains(x))).cloned().collect();
        }
    }
    count
}
