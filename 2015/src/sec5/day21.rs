...
fn equip(items: &Vec<&&Item>) -> Fighter {
    Fighter {
        hit_points: 100,
...
fn p1(input: &str) -> usize {
    let data = input.lines().map(|x| x.split(":").nth(1).unwrap().trim().parse::<usize>().unwrap()).collect_vec();
    let boss = Fighter {
        hit_points: data[0],
        damage: data[1],
        armor: data[2],
    ...
    let armor = armor();
    let rings = rings();
    let weapon_choices: Vec<Vec<&Item>> = (1..=1).flat_map(|x| weapons.iter().combinations(x)).collect_vec();
    let armor_choices: Vec<Vec<&Item>> = (0..=1).flat_map(|c| armor.iter().combinations(c)).collect_vec();
    let ring_choices: Vec<Vec<&Item>> = (0..=2).flat_map(|c| rings.iter().combinations(c)).collect_vec();

    [weapon_choices, armor_choices, ring_choices].iter()
        .multi_cartesian_product()
        .map(|c: Vec<&Vec<&Item>>| {
            c.iter().flat_map(|&x| x).collect_vec()
